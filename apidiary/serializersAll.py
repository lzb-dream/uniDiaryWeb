...
class Us(serializers.Serializer):
    ...
    diaryPassword = serializers.IntegerField(required=False)
    ...
    def validate_diaryPassword(self,value):
        if not isinstance(value,int):
            raise serializers.ValidationError('日记密码不全是数字类型')
        else:
            return value
...
class Ds(serializers.Serializer):
    ...
    videoPhoto = serializers.CharField(allow_blank=True, required=False)
